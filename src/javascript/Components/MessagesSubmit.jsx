...
function MessagesSubmit() {
    ...
    return (
        <div id="MessagesSubmit">
            ...
            <form className="w-full max-w-lg">
                <div className="flex flex-wrap -mx-3 mb-6">
                    <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input className="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="senderName" type="text" defaultValue={defaultValue} />
                    </div>
                    <div className="w-full md:w-1/2 px-3">
                    <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="receiverMail">
                            Receiver mail
                        </label>
                        <input className="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="receiverMail" type="text" placeholder="name@mail.com" />
                    </div>
                </div>
                <div className="flex flex-wrap -mx-3 mb-6">
            ...
            <button  
                id="complimentButton"
                className="bg-orange-700 hover:bg-orange-500 text-white font-bold py-2 px-4 rounded m-2 capitalize"
                onClick={handlePicture}
                >See a compliment</button>
            {/* Create a button for each language from LANGUAGES_ARRAY */}
            {LANGUAGES_ARRAY.map((language, index) => {
                return (
                    <button
                        key={index}
                        className="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded m-2 capitalize"
